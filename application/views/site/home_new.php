                                ...
                                <table class="table card-table table-vcenter text-center" id="stages_list">
                                    ...
                                    <tbody>
                                        ...
                                        <tr v-if="stages.length >= 10">
                                            <th></th>
                                            <th><button type="button" class="btn btn-info  btn-sm mb-1"
                                                    v-on:click="stages_list.lim += 100;"><?=lang('txt79')?></button>
                                            </th>
                                        </tr>
                                    </tbody>
                                </table>
    ...
    methods: {
        ...
        async update() {
           /* if (this.loading) {
            ...
            this.loading = false
            return res;*/
            this.user_bal_usd = data_s.user_bal_usd
            this.buy_stage = data_s.buy_stage
            this.sell_stage = data_s.sell_stage
            this.errors = data_s.errors;
        },
...
var stages_list = new Vue({
 ...
 var user_closed_orders_list = new Vue({
            ...
            methods: {
                ...
                build(){
                    ...
                    if(this.chart_data!=false){
                        //build_LightweightChart('flotChart_new',this.chart_data) 
                        build_apex_hart('flotChart_new',this.chart_data) 
                    }
                },
            },
            async mounted() {
...
<script>
    ...
    $(document).ready(async function() {
        let f_res = await get_update()
        rate_chart.chart_data = f_res.rate_chart;
        rate_chart.build()
    ...
    async function get_update() {
            ...
            loading = true
            let res = await request('api/HomeApiController/home/', 'POST', {
                buy_tok: buy_order.inputs.buy_tok,
                buy_price: buy_order.buy_stage.price,
                sell_price: buy_order.inputs.sell_price,

                orders_list_lim:orders_list.lim,
                user_orders_list_lim:user_orders_list.lim,
                stages_list_lim:stages_list.lim,
                user_closed_orders_list_lim:user_closed_orders_list.lim,

                get_chart: (!rate_chart.chart_data)?1:0 , 
            ...
            buy_order.sell_stage = res.sell_stage
            buy_order.errors = res.errors;

            orders_list.orders = res.orders_list;
